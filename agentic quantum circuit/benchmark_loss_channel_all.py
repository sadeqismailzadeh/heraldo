...
print("="*80)
print(" Comparing Optimized Loss vs LossMeasureFock vs MonitoredLossMeasureFock for Random Sampling with Large Cutoff ")
print("="*80)
...
print("[0/4] Benchmarking NO LOSS (just MeasureFock)...")
# No need to revert any patches since we're starting fresh
is_pure = []
timings_no_loss = []
...
# Original
print("[1/4] Benchmarking ORIGINAL Loss channel...")
from loss_channel_patch import revert_patch as revert_loss
revert_loss()
...
# Optimized Loss
print("[2/4] Benchmarking OPTIMIZED Loss channel...")
from loss_channel_patch import patch_loss_channel
patch_loss_channel()
...
# LossMeasureFock
print("[3/4] Benchmarking LossMeasureFock ...")
...
# Warmup JIT
print("  JIT warmup...")
warmup3 = sf.Program(1)
with warmup3.context as q:
    Dgate(0.1) | q[0]
    MonitoredLossMeasureFock(eta=0.5) | q[0]
eng_warmup3 = sf.Engine("fock", backend_options={"cutoff_dim": 5})
eng_warmup3.run(warmup3)
print("  Warmup done")
...
for trial in range(n_trials + 1):  # +1 for warmup
    ...
    with prog.context as q:
        Sgate(r_squeeze, phi_squeeze) | q[1]
        BSgate(theta_bs, 0) | (q[0], q[1])
        MonitoredLossMeasureFock(eta=eta) | q[0]
        BSgate(np.pi/2, 0) | (q[0], q[1])
    ...
    if trial > 0:  # Skip first trial
        timings_monitored.append(elapsed)

time_monitored = np.mean(timings_monitored)
std_monitored = np.std(timings_monitored)
print(f"  Time: {time_monitored*1000:.2f} +/- {std_monitored*1000:.2f} ms (excluding first trial)\n")

revert_monitored_patch()

# Summary
print("="*80)
print(" RESULTS ")
print("="*80)
print(f"\n{'Method':<35} {'Time (ms)':<15} {'Speedup':<12} {'Overhead':<10}")
print("-" * 90)
print(f"{'No Loss (baseline)':<35} {time_no_loss*1000:>10.2f}     {'-':<12} {'-':<10}")
print(f"{'Original Loss channel':<35} {time_orig*1000:>10.2f}     {'1.00x':<12} {time_orig/time_no_loss:>6.2f}x")
print(f"{'Optimized Loss channel':<35} {time_opt_loss*1000:>10.2f}     {time_orig/time_opt_loss:>5.2f}x      {time_opt_loss/time_no_loss:>6.2f}x")
print(f"{'LossMeasureFock':<35} {time_lmf*1000:>10.2f}     {time_orig/time_lmf:>5.2f}x      {time_lmf/time_no_loss:>6.2f}x")
print(f"{'MonitoredLossMeasureFock':<35} {time_monitored*1000:>10.2f}     {time_orig/time_monitored:>5.2f}x      {time_monitored/time_no_loss:>6.2f}x")
...
print(f"Loss Channel Overhead (MonitoredLoss): {(time_monitored - time_no_loss)*1000:.2f} ms ({(time_monitored/time_no_loss - 1)*100:.1f}% increase)")
print(f"\nOverhead Reduction: {(time_orig - time_no_loss)/(time_lmf - time_no_loss):.2f}x vs LossMeasureFock")

print("\n" + "="*80)
print(" IMPROVEMENT ")
print("="*80)
print(f"LossMeasureFock vs Optimized Loss: {time_opt_loss/time_lmf:.2f}x faster")
print(f"MonitoredLoss vs Optimized Loss:   {time_opt_loss/time_monitored:.2f}x faster")
print(f"LossMeasureFock vs Original:       {time_orig/time_lmf:.2f}x faster")
print(f"MonitoredLoss vs Original:         {time_orig/time_monitored:.2f}x faster")
