"""
Reproducing Table 1: Static Spatial Circuit Beam Search Pattern Discovery Sweeps.

This script executes the unconstrained Beam Search pattern discovery phase for all
static continuous-variable (CV) spatial photonic circuit configurations reported in
Table 1 of the paper: "Multi-Outcome Circuit Optimization for Enhanced Non-Gaussian State Generation".
...
Table 1 Configurations Evaluated (Static Spatial Models)
---------------------------------------------------------
1. **GKP core ($\mu=1$, 2 modes)**: TwoModeStaticSqueezeOnly targeting $n_{\text{max}} \in \{4, 6, 8, 10\}$ logical 1 GKP core states.
2. **GKP core ($\mu=1$, 3 modes)**: ThreeModeStaticSqueezeOnly targeting $n_{\text{max}} \in \{4, 6, 8, 10\}$ logical 1 GKP core states across 2 ancillae.
3. **Cat States (2 modes)**: TwoModeStaticSqueezeOnly targeting even ($\alpha=\sqrt{6}, p=0$) and odd ($\alpha=\sqrt{6}, p=1$) squeezed cat states.
4. **Cat States (3 modes)**: ThreeModeStaticSqueezeOnly targeting even and odd squeezed cat states across 2 ancillae.
5. **GKP core ($\mu=0$, 3 modes)**: ThreeModeStaticSqueezeOnly targeting $n_{\text{max}} \in \{4, 8, 12\}$ logical 0 GKP core states.
6. **Binomial Codes (3 modes)**: ThreeModeStaticSqueezeOnly targeting $(N=2, S=2)$ and $(N=2, S=3)$ logical zero binomial codewords.
7. **Cubic Phase State (3 modes)**: ThreeModeStaticGeneral targeting displaced approximate cubic phase state ($\gamma=-0.2, r=-0.7, \alpha=1.25$).

Generated Output & Artifacts
----------------------------
Results are exported to: ``results/sweeps_static_beam_search_<Timestamp>/``
Directory layout:
...
.. code-block:: bash
    ...
    python scripts/optimize/run_static_beam_search_sweeps.py
"""
...
def main() -> None:
    """Executes the automated Beam Search pattern discovery sweep for static spatial circuits.

    Initializes static spatial circuit parameters (12 dB source squeezing, cutoff dimension D=30,
    beam width B=200), constructs the 7 static sweep jobs corresponding to Table 1 of the paper,
    executes Basin-Hopping beam search optimizations sequentially across parallel worker pools, and
    compiles the final master Markdown report ``beam_search_report.md``.
    """
    CUTOFF_DIM = 30
    ...
    print("--- Defining Static Beam Search Discovery Jobs (Table I - Static Circuits) ---")

    sweep_jobs = []

    # =========================================================================
    # STATIC BEAM SEARCH PATTERN DISCOVERY JOBS (TABLE 1 OF THE PAPER)
    # =========================================================================
    ...
    sweep_jobs.append({
        ...
        "modes": 3,
        "circuit_config": {
            'class_name': 'ThreeModeStaticGeneral',
            'params': {
                'clip_size': squeezing,
                'measure_fock_cutoff': MEASURE_CUTOFF,
                'num_single_photon': 0,
    ...
    results_dir = Path(__file__).resolve().parent.parent.parent / "results" / f"sweeps_static_beam_search_{timestamp}"
    results_dir.mkdir(parents=True, exist_ok=True)
    print(f"Static Beam Search sweep results will be written to: {results_dir}\n")
    ...
    for idx, job in enumerate(sweep_jobs):
        print("=" * 80)
        print(f"STATIC BEAM SEARCH JOB {idx+1}/{len(sweep_jobs)}: Family={job['family']} | Modes={job['modes']}")
        print("=" * 80)
        ...
        try:
            ...
            # Instantiate static BasinHoppingRunner with measurement_patterns=None for BEAM SEARCH
            ...
            res = runner.run(n_iter=N_GENERATIONS)
            ...
            try:
                ...
                if 'x' in res:
                    ...
                    np.savez(best_dir / "mapped_params.npz", mapped_params=res['x'])

                    schedule = {
                        "param_names": param_names,
                        "mapped_params": res['x'].tolist(),
                        "circuit_config": job['circuit_config'],
                        "target_configs": job['target_configs']
                    }
    ...
    print("\n" + "=" * 80)
    print("ALL STATIC BEAM SEARCH JOBS COMPLETE - COMPILING REPORT")
    print("=" * 80)

    report_path = results_dir / "beam_search_report.md"
    with open(report_path, "w") as f:
        f.write("# Static Beam Search Pattern Discovery Report (Table I)\n\n")
